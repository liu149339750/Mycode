...
/*    	mSuccessAnimation.setObjectValues(p1,p2,p3,p1);
    	...
    	mSuccessAnimation.setEvaluator(new TypeEvaluator<PointF>() {
    	...
    	mSuccessAnimation.addListener(new AnimatorListener() {
...
//                float progress = ((mCenterX + mRadius / 2 + mArrowLineToHorizontalLineAnimatedValue / 2) - (mCenterX - mRadius / 2 - mArrowLineToHorizontalLineAnimatedValue / 2)) / 360f;
                ...
                canvas.drawText(""+progress, mCenterX, mCenterY - (mProgressTextPaint.descent() + mProgressTextPaint.ascent())/2, mProgressTextPaint);
...
//                );
                canvas.save();
                canvas.translate(mCenterX-mRadius/4,  mCenterY + (float)Math.sin(Math.toRadians(60))*mRadius/2);
...
//        }
    ...
    public void setErrorResultState() {
        if(mSucessAnimatorSet.isRunning() || mErrorAnimation.isRunning())
            return;
        mResultState = State.ANIMATING_ERROR;
    }
